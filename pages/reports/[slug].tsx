...
export default function ReportPage({ title, content, author, date, tags }: ReportPageProps) {
  return (
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  try {
    ...
    const reports = await fetchReportsFromGitHub();
    const report = reports.find(file => file?.name === `${slug}.md`);
    ...
    const { frontMatter, content } = await processMarkdown(report.content);
